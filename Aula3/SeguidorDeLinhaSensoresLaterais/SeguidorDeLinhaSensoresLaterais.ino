// Robô Seguidor de Linha Repouso com Controle Proporcional e Derivativo - Arduino Nano
...
// Pinos dos sensores frontais (A1 a A6)
int pinoSensores[NUM_SEN] = {A1, A2, A3, A4, A5, A6};

// Pinos dos sensores lateriais (A0 e A7)
int sensorDir = A0;
int sensorEsq = A7;
...
//Variáveis de Tempo
unsigned long ultimoTempoDir = 0;
const unsigned long intervaloDir = 50; // 50 milissegundos

unsigned long ultimoTempoEsq = 0;
const unsigned long intervaloEsq = 100; // 100 milissegundos

// Definir a velocidade pelos trajetos
#define N_FAIXA 0 // Coloque a quantidade de faixas da esquerda + 1
int velTrajeto[N_FAIXA] = {}; // Coloque as velocidades para cada um dos trajetos
...
void calibrarSensores() {
  ...
  while (millis() - tempoInicial < 3000) {
    ...
    int faixaEsq = analogRead(sensorEsq);
    if (faixaEsq < minEsq) minEsq = faixaEsq;
    if (faixaEsq > maxEsq) maxEsq = faixaEsq;

    int faixaDir = analogRead(sensorDir);
    if (faixaDir < minDir) minDir = faixaDir;
    if (faixaDir > maxDir) maxDir = faixaDir;
  ...
  Serial.print(minEsq);
  Serial.print(", Max=");
  Serial.println(maxEsq);

  Serial.println("Sensor Direito:");
  Serial.print(": Min=");
  Serial.print(minDir);
  Serial.print(", Max=");
  Serial.println(maxDir);
...
void seguirLinha() {
  ...
  lerSensores();
  lerSensorLateral(sensorEsq);
  lerSensorLateral(sensorDir);

  unsigned long agora = millis();

  Serial.println(valorDir);
  if (valorDir < 230 && (agora - ultimoTempoDir >= intervaloDir)) {
    N_Dir++;
    ultimoTempoDir = agora; // Atualiza o tempo da última contagem
  }

  Serial.println(valorEsq);
  if(valorEsq < 230 && (agora - ultimoTempoEsq >= intervaloEsq)){
    N_Esq++;
    ultimoTempoEsq = agora; // Atualiza o tempo da ultima contagem
  }
  ...
  int velEsquerda = velTrajeto[N_Esq] + pid;
  int velDireita = velTrajeto[N_Esq] - pid;
...
void lerSensorLateral(int sensorLateral){
  int leitura = analogRead(sensorLateral);

  // Determinar qual sensor está sendo lido para usar calibração correta
  if (sensorLateral == sensorEsq) {
    // Normalizar entre 0 e 1000
    valorEsq = map(leitura, minEsq, maxEsq, 0, 1000);
    valorEsq = constrain(valorEsq, 0, 1000);
  } else {
    // Normalizar entre 0 e 1000
    valorDir = map(leitura, minDir, maxDir, 0, 1000);
    valorDir = constrain(valorDir, 0, 1000);
  }
}
